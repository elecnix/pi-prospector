...
async function main() {
	...
	// Get sessions with high friction or corrections
	const sessions = db.prepare(`
		SELECT DISTINCT session_id FROM analysis_nodes 
		WHERE analyzer_id = 'turn-pair-core' AND (json_extract(content_json, '$.correction_detected') = 1 OR json_extract(content_json, '$.friction_score') > 0.3)
	`).all();
	
	console.log(`Found ${sessions.length} sessions needing LLM analysis`);
	...
	for (const { session_id } of sessions) {
		try {
			...
			if (analyzed % 5 === 0) console.log(`Progress: ${analyzed} sessions...`);
		} catch (e) {
			errors++;
		}
	}
	
	console.log(`\nAnalyzed ${analyzed} sessions, skipped ${skipped} (idempotency), ${errors} errors`);
	
	// Summary
	const summary = db.prepare(`
		SELECT 
			json_extract(content_json, '$.sentiment') as sentiment,
			COUNT(*) as count
		FROM analysis_nodes 
		WHERE analyzer_id = 'turn-pair-llm'
		GROUP BY sentiment
	`).all();
	console.log("\nSentiment distribution:", JSON.stringify(summary));
