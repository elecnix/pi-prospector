...
- `src/analyze/analyzers/<id>/` — one directory per analyzer (`turn-pair-core`, `turn-pair-llm`, `session-overview`, `rule-restatement`, `tool-trajectory`, `failure-modes`, `secret-leak`), each with `index.ts`, its prompt(s), and `config.ts`.
