...
export function getCursor(db: Database.Database, filePath: string): { last_line: number; last_modified: number } | undefined {
	return prepare(db, "SELECT last_line, last_modified FROM sessions WHERE file_path = ?").get(filePath) as { last_line: number; last_modified: number } | undefined;
}

export function updateCursor(db: Database.Database, sessionId: string, lastLine: number, lastModified: number): void {
	prepare(db, "UPDATE sessions SET last_line = ?, last_modified = ? WHERE id = ?").run(lastLine, lastModified, sessionId);
}

export function updateMessageCount(db: Database.Database, sessionId: string, count: number): void {
	prepare(db, "UPDATE sessions SET message_count = ? WHERE id = ?").run(count, sessionId);
}

export function markAnalyzed(db: Database.Database, sessionId: string): void {
	prepare(db, "UPDATE sessions SET analyzed_at = ? WHERE id = ?").run(new Date().toISOString(), sessionId);
}
...
export function getUnanalyzedSessions(db: Database.Database, limit?: number): Array<{ id: string; file_path: string; started_at: string }> {
	...
	return (limit ? prepare(db, sql).all(limit) : prepare(db, sql).all()) as Array<{ id: string; file_path: string; started_at: string }>;
}
...
export function getSessionLabels(db: Database.Database): SessionLabel[] {
	return prepare(db, "SELECT id, project, cwd, message_count FROM sessions").all() as SessionLabel[];
}
...
export function countMessages(db: Database.Database, sessionId: string): number {
	return (prepare(db, "SELECT COUNT(*) as c FROM messages WHERE session_id = ?").get(sessionId) as { c: number }).c;
}
...
export function getSessionMessages(db: Database.Database, sessionId: string): Array<{ role: string; content_text: string | null; content_thinking: string | null; tool_calls: string | null; timestamp: string | null }> {
	return prepare(db, "SELECT role, content_text, content_thinking, tool_calls, timestamp FROM messages WHERE session_id = ? ORDER BY rowid ASC").all(sessionId) as any[];
}
...
export function listProposals(db: Database.Database, status?: string, severity?: string, limit?: number, offset?: number): Proposal[] {
	...
	return prepare(db, sql).all(...params) as Proposal[];
}

export function getProposal(db: Database.Database, id: string): Proposal | undefined {
	return prepare(db, "SELECT * FROM proposals WHERE id = ?").get(id) as Proposal | undefined;
}
...
function decideProposal(
	...
	const row = prepare(db, "SELECT input_key, status FROM proposals WHERE id = ?").get(id) as
		| { input_key: string; status: string }
		| undefined;
	if (!row || row.status !== "open") return false;
	const now = new Date().toISOString();
	const tx = db.transaction(() => {
		prepare(db, "UPDATE proposals SET status = ?, updated_at = ? WHERE id = ?").run(newStatus, now, id);
		prepare(
			db,
	...
	const tx = db.transaction(() => {
		const open = new Set(
			proposalIds.filter((id) => {
				const row = prepare(db, "SELECT status FROM proposals WHERE id = ?").get(id) as { status: string } | undefined;
				return row?.status === "open";
			}),
		);
		if (open.size > 0) {
			remediationId = uuidv7();
			prepare(db, "INSERT INTO remediations (id, description, actual_change, created_at) VALUES (?, ?, ?, ?)").run(
				remediationId,
...
export function getRemediation(db: Database.Database, id: string): Remediation | undefined {
	return prepare(db, "SELECT * FROM remediations WHERE id = ?").get(id) as Remediation | undefined;
}
...
export function getDecisionsForRemediation(db: Database.Database, remediationId: string): ProposalDecision[] {
	return prepare(
		db,
		"SELECT * FROM proposal_decisions WHERE remediation_id = ? ORDER BY decided_at ASC, rowid ASC")
...
export function getLatestDecision(db: Database.Database, proposalInputKey: string): ProposalDecision | undefined {
	return prepare(
		db,
		"SELECT * FROM proposal_decisions WHERE proposal_input_key = ? ORDER BY decided_at DESC, rowid DESC LIMIT 1")
...
export function getDecisionsForProposal(db: Database.Database, proposalInputKey: string): ProposalDecision[] {
	return prepare(
		db,
		"SELECT * FROM proposal_decisions WHERE proposal_input_key = ? ORDER BY decided_at ASC, rowid ASC")
...
export function getAllDecisions(db: Database.Database): ProposalDecision[] {
	return prepare(db, "SELECT * FROM proposal_decisions ORDER BY decided_at DESC, rowid DESC").all() as ProposalDecision[];
}
...
export function listOpenProposalsForSession(db: Database.Database, sessionId: string): Proposal[] {
	return prepare(
		db,
		"SELECT * FROM proposals WHERE session_id = ? AND status = 'open' ORDER BY created_at ASC, rowid ASC")
...
export function listSessionIdsWithOpenProposals(db: Database.Database, limit?: number): string[] {
	const rows = prepare(
		db,
		"SELECT DISTINCT session_id FROM proposals WHERE status = 'open' ORDER BY session_id")
...
export function countOpenProposalsByValidationStatus(db: Database.Database): Record<string, number> {
	const rows = prepare(
		db,
		"SELECT validation_status AS s, COUNT(*) AS c FROM proposals WHERE status = 'open' GROUP BY validation_status")
...
export function getStats(db: Database.Database): Stats {
	const totalSessions = (prepare(db, "SELECT COUNT(*) as c FROM sessions").get() as { c: number }).c;
	const piSessions = (prepare(db, "SELECT COUNT(*) as c FROM sessions WHERE source = 'pi'").get() as { c: number }).c;
	const claudeSessions = (prepare(db, "SELECT COUNT(*) as c FROM sessions WHERE source = 'claude'").get() as { c: number }).c;
	const totalMessages = (prepare(db, "SELECT COUNT(*) as c FROM messages WHERE role IN ('user','assistant')").get() as { c: number }).c;
	const piMessages = (prepare(db, "SELECT COUNT(*) as c FROM messages WHERE role IN ('user','assistant') AND source = 'pi'").get() as { c: number }).c;
	const claudeMessages = (prepare(db, "SELECT COUNT(*) as c FROM messages WHERE role IN ('user','assistant') AND source = 'claude'").get() as { c: number }).c;
	const totalToolResults = (prepare(db, "SELECT COUNT(*) as c FROM messages WHERE role = 'toolResult'").get() as { c: number }).c;
	const sessionsAnalyzed = (prepare(db, "SELECT COUNT(*) as c FROM sessions WHERE analyzed_at IS NOT NULL").get() as { c: number }).c;

	const statusRows = prepare(db, "SELECT status, COUNT(*) as c FROM proposals GROUP BY status").all() as Array<{ status: string; c: number }>;
