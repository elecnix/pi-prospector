...
async function seed(db: AsyncDatabase): Promise<void> {
...
async function fillV1(db: AsyncDatabase): Promise<void> {
...
async function run(db: AsyncDatabase, analyzer: Analyzer, revise: ReviseReason[]) {
	const fw = new AnalyzerFramework({ db, llm: createThrowingLLM(), modelTiers: DEFAULT_MODEL_TIERS });
	await fw.register(analyzer);
	return fw.run("s1", { revise });
}

async function scan(db: AsyncDatabase, analyzer: Analyzer) {
	const fw = new AnalyzerFramework({ db, llm: createThrowingLLM(), modelTiers: DEFAULT_MODEL_TIERS });
	await fw.register(analyzer);
	return fw.scan("s1");
}
