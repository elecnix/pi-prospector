...
function newFramework(db: import("better-sqlite3").Database) {
	...
	fw.register(detectSecretsAnalyzer);
	return fw;
}

function readNode(
	db: import("better-sqlite3").Database,
): { row: Record<string, unknown>; props: DetectSecretsProperties } {
	const rows = db
...
describe("detect-secrets component test", () => {
	it("detects a secret in a user message, anchors to session and message, and stores no full secret", async () => {
		...
		try {
			insertSession(db, "ds-1");
			const ids = insertMessages(db, "ds-1", [
	...
	it("is idempotent: a second run produces no new node", async () => {
		const { db, close } = tempDb();
		try {
			insertSession(db, "ds-2");
			insertMessages(db, "ds-2", [
	...
	it("re-identifies when the session grows new turns and rescans", async () => {
		const { db, close } = tempDb();
		try {
			insertSession(db, "ds-3");
			insertMessages(db, "ds-3", [
	...
	it("detects a candidate inside a tool_result field", async () => {
		const { db, close } = tempDb();
		try {
			insertSession(db, "ds-4");
			const ids = insertMessages(db, "ds-4", [
			...
			const summary = await newFramework(db).run("ds-4", { analyzerIds: ["detect-secrets"] });
	...
	it("applies exclusion filters in the live pipeline (placeholders produce no findings)", async () => {
		const { db, close } = tempDb();
		try {
			insertSession(db, "ds-5");
			insertMessages(db, "ds-5", [
			...
			const summary = await newFramework(db).run("ds-5", { analyzerIds: ["detect-secrets"] });
			assert.equal(summary.errors.length, 0);
			const { props } = readNode(db);
	...
	it("clean session produces a node with has_leaks=false", async () => {
		const { db, close } = tempDb();
		try {
			insertSession(db, "ds-6");
			insertMessages(db, "ds-6", [
				{ role: "user", text: "refactor the helpers please" },
				{ role: "assistant", text: "splitting them by concern" },
			] satisfies TestMessage[]);

			const summary = await newFramework(db).run("ds-6", { analyzerIds: ["detect-secrets"] });
			assert.equal(summary.errors.length, 0);
