...
async function newFramework(db: import("better-sqlite3").Database) {
...
async function readNode(db: import("better-sqlite3").Database): Promise<{ row: Record<string, unknown>; props: SecretLeakProperties }> {
	const rows = (await db
		.prepare("SELECT * FROM analysis_nodes WHERE analyzer_id = ? ORDER BY created_at ASC")
		.all(SECRET_LEAK_DEF.id)) as unknown as Array<Record<string, unknown>>;
...
describe("secret-leak component test", () => {
	it("detects a secret in a user message, anchors to session and message, and stores no full secret", async () => {
		...
		try {
			...
			const ids = await insertMessages(db, "leak-1", [
	...
	it("detects a PEM private key inside a tool_result field", async () => {
		...
		try {
			...
			const ids = await insertMessages(db, "leak-4", [
			...
			const summary = await (await newFramework(db)).run("leak-4", { analyzerIds: ["secret-leak"] });
	...
	it("clean session produces a node with has_leaks=false", async () => {
		...
		try {
			...
			const summary = await (await newFramework(db)).run("leak-5", { analyzerIds: ["secret-leak"] });
			assert.equal(summary.errors.length, 0);
