...
async function newFramework(db: AsyncDatabase) {
	...
	await fw.register(noseyParkerAnalyzer);
	return fw;
}

async function readNode(
	db: AsyncDatabase,
): Promise<{ row: Record<string, unknown>; props: NoseyParkerProperties }> {
	const rows = await db
...
describe("nosey-parker component test", () => {
	it("detects a secret in a user message, anchors to session and message, and stores no full secret", async () => {
		...
		try {
			await insertSession(db, "np-1");
			const ids = await insertMessages(db, "np-1", [
	...
	it("is idempotent: a second run produces no new node", async () => {
		const { db, close } = await tempDb();
		try {
			await insertSession(db, "np-2");
			await insertMessages(db, "np-2", [
	...
	it("re-identifies when the session grows new turns and rescans", async () => {
		const { db, close } = await tempDb();
		try {
			await insertSession(db, "np-3");
			await insertMessages(db, "np-3", [
	...
	it("detects a captured secret inside a tool_result field", async () => {
		const { db, close } = await tempDb();
		try {
			await insertSession(db, "np-4");
			const ids = await insertMessages(db, "np-4", [
			...
			const summary = await (await newFramework(db)).run("np-4", { analyzerIds: ["nosey-parker"] });
	...
	it("clean session produces a node with has_leaks=false", async () => {
		const { db, close } = await tempDb();
		try {
			await insertSession(db, "np-5");
			await insertMessages(db, "np-5", [
				{ role: "user", text: "refactor the helpers please" },
				{ role: "assistant", text: "splitting them by concern" },
			] satisfies TestMessage[]);

			const summary = await (await newFramework(db)).run("np-5", { analyzerIds: ["nosey-parker"] });
			assert.equal(summary.errors.length, 0);
