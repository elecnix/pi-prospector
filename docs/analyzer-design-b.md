...
| `emits` | analysis_node | (proposal extracted from content) | This node produced this proposal. |
    ...
    WHERE e.to_ref_kind = 'message' AND e.edge_kind = 'anchors'
      AND e.from_node_id IN (
          ...
          WHERE edge_kind = 'consumes' AND to_ref_kind = 'analysis_node'
            AND to_ref_id IN (
                SELECT from_node_id FROM analysis_edges
                WHERE edge_kind = 'emits' AND to_ref_id = 'proposal_42'
            )
      )
    UNION ALL
    SELECT e.from_node_id, e.to_ref_kind, e.to_ref_id
...
interface AnalysisResult {
    ...
    edges: Array<{
        ...
        toRefId: string;
        edgeKind: 'anchors' | 'consumes' | 'refines' | 'uses_prompt' | 'uses_config' | 'emits';
        ordinal?: number;
    }>;
...
emits    → proposal nodes (materialized into proposals table)
```
...
2. For each proposal:
   ...
   d. If no → INSERT into `proposals` and create analysis edges:
      - `analysis_edges(from_node_id=proposal_node, to_ref_kind='analysis_node', to_ref_id=source_analysis_node, edge_kind='emits')`
